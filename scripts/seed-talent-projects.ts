...
// S3 Base URL
const S3_BASE_URL =
  "https://art-gallery-s3-bucket.s3.eu-north-1.amazonaws.com/images";
...
async function main() {
  ...
  if (talentTypes.length === 0) {
    console.log(
      "❌ No talent types found. Please run the main seed script first."
    );
    return;
  }
  ...
  for (const talentType of talentTypes) {
    ...
    if (artists.length === 0) {
      console.log(
        `   ⚠️  No artists found for ${talentType.name}, creating one...`
      );
    ...
    const titles =
      artworkTitlesByTalent[talentType.slug] || artworkTitlesByTalent.other;
    const relevantCategories = categoryMapping[talentType.slug] || [
      "mixed-media",
    ];
    ...
    for (const artist of artists) {
      for (let i = 0; i < artworksPerArtist; i++) {
        ...
        try {
          ...
          console.log(
            `   ✅ Created artwork: "${artwork.title}" by ${artist.name} (€${artwork.desiredPrice})`
          );
        } catch (error: any) {
          if (error.code === "P2002") {
    ...
    console.log(
      `   📊 Created ${artworksPerArtist} artworks per artist for ${talentType.name}`
    );
  }
