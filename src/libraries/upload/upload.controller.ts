...
export class UploadController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Presigned URL generated successfully. Returns presignedUrl (for uploading), publicUrl (for accessing after upload), and objectKey (S3 path).',
  ...
  @ApiOperation({
    ...
    description: 'Generate presigned URLs for uploading multiple images (up to 10) directly to S3 from the frontend.',
  })
  @ApiBody({ type: GenerateMultiplePresignedUrlsDto })
  @ApiResponse({
    status: 200,
    description: 'Presigned URLs generated successfully. Returns array of presigned URLs for multiple file uploads.',
  ...
  async getPresignedImageUrls(@Body() dto: GenerateMultiplePresignedUrlsDto) {
    const results = await this.uploadService.getPresignedMultipleImageUploadUrls(
      dto.files,
      dto.expirySeconds,
    );
  ...
  @ApiOperation({
    ...
    description: 'Generate a presigned URL for uploading a document (PDF, JPEG, PNG) directly to S3 from the frontend.',
  })
