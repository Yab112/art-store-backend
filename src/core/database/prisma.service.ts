    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (error.code === 'P1001') {
          ...
          // Last attempt failed
          this.logger.error('❌ Failed to connect to database after retries:', error.message);
          this.logger.error('Database URL:', process.env.DATABASE_URL ? 'Set (hidden)' : 'Not set');
          this.logger.error('Error code:', error.code);
          this.logger.error('');
          this.logger.error('⚠️  Cannot reach database server. Possible causes:');
          this.logger.error('1. Database server is paused (Neon free tier auto-pauses after inactivity)');
          this.logger.error('   → Solution: Go to https://console.neon.tech and wake up your database');
          ...
          this.logger.error('');
          this.logger.error('💡 Quick fix: Visit your Neon dashboard and ensure the database is active, then restart the server.');
        } else {
