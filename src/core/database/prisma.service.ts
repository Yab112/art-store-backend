  ...
  async onModuleInit(): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        if (attempt > 1) {
          this.logger.log(
            `Retrying database connection (attempt ${attempt}/${maxRetries})...`,
          );
        ...
        if (error.code === "P1001") {
          ...
          if (!isLastAttempt) {
            this.logger.warn(
              `Database connection failed (attempt ${attempt}/${maxRetries}). This might be due to Neon auto-pause. Retrying...`,
            );
            continue;
          }
