...
export class PaymentController {
  ...
  @Post("paypal/webhook")
  @HttpCode(HttpStatus.OK)
  async paypalWebhook(@Body() payload: any, @Headers() headers: any) {
    try {
      ...
      if (payload && typeof payload === "string" && payload.includes("ngrok")) {
        this.logger.error(
          `Received ngrok browser warning page instead of webhook payload`,
        );
      ...
      const result = await this.paymentService.handlePaypalWebhook(
        payload,
        headers,
      );
