...
export class OrderController {
  ...
  async getMyOrders(@CurrentUser() user: any) {
    ...
    this.logger.log(
      `Get orders for authenticated user: ${user.id}, email: ${user.email || "N/A"}`,
    );
  ...
  async createOrder(
    ...
    if (user && user.id) {
      userId = user.id;
      this.logger.log(
        `✅ User authenticated: ${userId} (${user.email || "N/A"})`,
      );
    } else {
      // Fallback: try to get user by email (for backward compatibility)
      this.logger.warn(
        `⚠️ No user in session, trying to find by email: ${createOrderDto.buyerEmail}`,
      );
      try {
    ...
    if (!userId || userId === "guest") {
      ...
      throw new UnauthorizedException(
        "You must be logged in to create an order. Please sign in and try again.",
      );
    }
